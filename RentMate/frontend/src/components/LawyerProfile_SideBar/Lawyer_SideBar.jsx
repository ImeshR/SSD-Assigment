...
const Lawyer_SideBar = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8080/api/lawyer/user/${id}`)
  ...
  async function deleteProfile() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(`http://localhost:8080/api/lawyer/${lawyer._id}`)
          .then((res) => {
        ...
        //delete from auth
        axios
          .delete(`http://localhost:8080/api/userr/${id}`)
          .then((res) => {
            console.log(res.data);
            localStorage.clear();
  ...
  return (
    <div className={styles.info_card}>
      <div className={styles.card1_back}>
        ...
        <h2>{!lawyer ? <p>Add Name</p> : <>{lawyer.name}</>}</h2>
        <h3>{user.email}</h3>
        <h3>
          {!lawyer ? <p>Add Contact Number</p> : <>{lawyer.contactNumber}</>}
        </h3>
      </div>
      <div className={styles.notification_card}>
        ...
        <div className={styles.sideicon}>
          ...
          <a href="#" className={styles.sidename}>
            <span className={styles.gradient} onClick={() => deleteProfile()}>
              Delete Profile
            </span>
          </a>
        </div>
